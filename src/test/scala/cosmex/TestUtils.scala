...
object CardanoInfoTestNet {
    ...
    /** Create CardanoInfo with current protocol parameters from provider
      ...
      *
      * For Blockfrost providers (preprod, preview, mainnet), this uses the correct SlotConfig for
      * the network, which is critical for time-to-slot conversions in transaction validity.
      *
      * @param provider
            ...
            case bf: BlockfrostProvider =>
                ...
                val (network, slotConfig) = bf.networkType match {
                    case BlockfrostProvider.NetworkType.Mainnet =>
                        println(
                          s"[CardanoInfoTestNet] Using Blockfrost Mainnet with SlotConfig.Mainnet"
                        )
                        (Network.Mainnet, SlotConfig.Mainnet)
                    case BlockfrostProvider.NetworkType.Preprod =>
                        println(
                          s"[CardanoInfoTestNet] Using Blockfrost Preprod with SlotConfig.Preprod"
                        )
                        (Network.Testnet, SlotConfig.Preprod)
                    case BlockfrostProvider.NetworkType.Preview =>
                        println(
                          s"[CardanoInfoTestNet] Using Blockfrost Preview with SlotConfig.Preview"
                        )
                        (Network.Testnet, SlotConfig.Preview)
                    case BlockfrostProvider.NetworkType.Unknown =>
                        println(
                          s"[CardanoInfoTestNet] Warning: Unknown Blockfrost network, defaulting to mainnet config"
                        )
